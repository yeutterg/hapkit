//--------------------------------------------------------------------------
// Code to compare parameters when simulating a hard surface with the hapkit
// May 27, 2015
...
// Select simulation
#define SURF_MIN           (char)'A'
#define SURF_MED           (char)'B'
#define SURF_MAX           (char)'C'

//defaults to
#define ENV_TYPE           SURF_MED
...
void loop()
{
  ...
  if(Serial.available())
  {
    ...
    {
      case SURF_MIN:
        Serial.println("Min Spring Constant");
      break;
      
      case SURF_MED:      
        Serial.println("Med Spring Constant");  
      break;
  
      case SURF_MAX:
        Serial.println("Max Spring Constant");
      break;
    }
  }
  ...
  double k_hard = 400; // N/m
  double x_wall_hard = 0.005; // m
  ...
  if(local_xh > x_wall_hard)
    {
      ...
      force = -k_hard * ( local_xh - x_wall_hard );
      if(!in_wall)
      {
        in_wall = true;
        impact_time = micros();
